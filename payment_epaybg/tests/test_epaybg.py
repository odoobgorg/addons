...
class epaybgCommon(PaymentAcquirerCommon):

    def setUp(self):
        super(epaybgCommon, self).setUp()
        cr, uid = self.cr, self.uid
        self.base_url = self.registry('ir.config_parameter').get_param(cr, uid, 'web.base.url')

        # get the epaybg account
        model, self.epaybg_id = self.registry('ir.model.data').get_object_reference(cr, uid, 'payment_epaybg', 'payment_acquirer_epaybg')
...
class epaybgServer2Server(epaybgCommon):

    def test_00_tx_management(self):
        cr, uid, context = self.cr, self.uid, {}


class epaybgForm(epaybgCommon):

    def test_10_epaybg_form_render(self):
        cr, uid, context = self.cr, self.uid, {}
        # be sure not to do stupid things
        epaybg = self.payment_acquirer.browse(self.cr, self.uid, self.epaybg_id, None)
        self.assertEqual(epaybg.environment, 'test', 'test without test environment')
        ...
        form_values = {
            ...
            'paymentAmount': '1',
            'currencyCode': 'EUR',
            'resURL': '%s' % urlparse.urljoin(self.base_url, EpaybgController._return_url),
        }

        # render the button
        res = self.payment_acquirer.render(
            cr, uid, self.epaybg_id,
        ...
        self.assertEqual(tree.get('action'), 'https://test.epaybg.com/hpp/pay.shtml', 'epaybg: wrong form POST url')
        for form_input in tree.input:
            ...
            self.assertEqual(
                ...
                'epaybg: wrong value for input %s: received %s instead of %s' % (form_input.get('name'), form_input.get('value'), form_values[form_input.get('name')])
            )

    # @mute_logger('openerp.addons.payment_epaybg.models.epaybg', 'ValidationError')
    # def test_20_paypal_form_management(self):
    #     cr, uid, context = self.cr, self.uid, {}
    #     # be sure not to do stupid things
    #     epaybg = self.payment_acquirer.browse(self.cr, self.uid, self.epaybg_id, None)
    #     self.assertEqual(epaybg.env, 'test', 'test without test env')
